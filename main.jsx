// main.jsx - Cliente React con Accesibilidad y Modularidad
...
// Componente de selección de país (accesible y reutilizable)
...
// Hook para cargar datos de álbumes
function useAlbums(country) {
  ...
  useEffect(() => {
    setLoading(true);
    setErr(null);
    axios.get(`https://spotify-dashboard-server.onrender.com/api/top-albums?country=${country}`)
...
// Hook para cargar distribución de géneros (pie chart)
function useGenres(country) {
  const [genres, setGenres] = useState([]);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    setLoading(true);
    axios.get(`https://spotify-dashboard-server.onrender.com/api/album-genres?country=${country}`)
      .then(res => {
        // Convierte objeto {genre: count} a array
        setGenres(Object.entries(res.data).map(([name, value]) => ({ name, value })));
      })
      .catch(() => setGenres([]))
      .finally(() => setLoading(false));
  }, [country]);
...
function AlbumsBarChart({ albums }) {
  // Usamos un valor fijo de popularidad si no existe (la API new-releases no da popularidad, esto es solo ejemplo)
...
function GenresPieChart({ genres }) {
